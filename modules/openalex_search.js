var OpenAlexSearch = {
    ...
    search: async function(query) {
        ...
        try {
            // 使用 search 参数（OR逻辑，靠客户端过滤）
            var url = 'https://api.openalex.org/works?search=' + encodeURIComponent(query) + '&per_page=50&sort=cited_by_count:desc';
    ...
    // 单独搜索顶刊
    searchVenue: async function(query, venueName) {
        var papers = [];
        console.log('[OpenAlex] 顶刊搜索:', query, '+ venue:', venueName);
        try {
            var url = 'https://api.openalex.org/works?search=' + encodeURIComponent(query + ' ' + venueName) + '&per_page=20&sort=cited_by_count:desc';
            var yr = document.getElementById('yr').value;
            if (yr) url += '&filter=publication_year:>' + (new Date().getFullYear() - parseInt(yr));
            var res = await fetch(url);
            var data = await res.json();
        ...
        console.log('[OpenAlex] 顶刊 ' + venueName + ': ' + papers.length + ' 篇');
        return papers;
    }
};