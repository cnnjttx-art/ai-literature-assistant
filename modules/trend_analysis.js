var TrendAnalysis = {
    ...
    summary: function(papers) {
        var yd = this.yearDist(papers), vd = this.venueDist(papers), kf = this.keywordFreq(papers,10), cs = this.citeStats(papers), sd = this.sourceDist(papers), ts = this.topStats(papers), yrs = yd.years;
        var lines = [];
        ...
        if (ts.venues.length>0) lines.push('顶刊来源: '+ts.venues.map(function(v){return v[0]+'('+v[1]+')'}).join(', '));
        lines.push('高频词: '+kf.map(function(w){return w[0]+'('+w[1]+')'}).join(', '));
        lines.push('期刊Top5: '+vd.slice(0,5).map(function(v){return v[0]+'('+v[1]+')'}).join(', '));
        lines.push('平均引用: '+cs.avg+', 总引用: '+cs.total);
        lines.push('数据来源: '+sd.map(function(s){return s[0]+': '+s[1]}).join(', '));
        return lines;
    }
};